...
const { embeddings } = await embedMany({
  ...
  // チャンクが多いとレートリミットに引っかかった。今回は動作見るだけなので数減らしとく
	values: [chunks[0]].map((chunk) => chunk.text),
	maxRetries: 0,
});
...
await vectorStore.createIndex({
	...
	dimension,
	indexConfig: {
    // gemini由来のembeddingはデフォで次元数3072みたい。これをそのまま保存できるようにするにはこれを指定する必要あり。
		type: "flat",
	},
});
await vectorStore.upsert({
	indexName,
	vectors: embeddings,
	metadata: [chunks[0]]?.map((chunk) => ({ text: chunk.text })),
});
